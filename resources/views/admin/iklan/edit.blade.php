                ...
                <form action="{{ route('admin.iklan.update', $iklan) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            ...
                            <input type="text" name="title" value="{{ old('title', $iklan->title) }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex items-center">
                                <img class="inline-block h-12 w-12 rounded bg-gray-100 mr-4" src="{{ Storage::url($iklan->image_path) }}" alt="{{ $iklan->title }}">
                        ...
                        <div>
                            ...
                            <select name="position" required class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @foreach($positions as $val => $label)
                                    <option value="{{ $val }}" {{ old('position', $iklan->position) == $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-2">
                            ...
                            <input type="url" name="url_link" value="{{ old('url_link', $iklan->url_link) }}" placeholder="https://..." class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        ...
                        <div>
                            ...
                            <input type="date" name="expires_at" value="{{ old('expires_at', $iklan->expires_at ? $iklan->expires_at->format('Y-m-d') : '') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            @error('expires_at') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div class="flex items-center space-x-3 mt-6">
                            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $iklan->is_active) ? 'checked' : '' }} class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                            <label for="is_active" class="text-sm font-medium text-gray-700">Iklan Ini Aktif</label>
                        </div>
                    </div>
