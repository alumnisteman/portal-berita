    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Perbarui informasi berita Anda untuk pembaca setia.</p>
            </div>
        </div>
    </x-slot>
                    ...
                    <form action="{{ route('admin.berita.update', $berita) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea name="content" rows="12" 
                                      ...
                                      placeholder="Tuliskan isi berita Anda..." required>{{ old('content', $berita->content) }}</textarea>
                            @error('content')
                                <p class="text-rose-500 text-xs mt-1 font-semibold ml-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="category_id" 
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ (old('category_id') ?? $berita->category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <!-- Image Upload -->
                            <div class="space-y-2">
                                <label class="block text-sm font-bold text-slate-700 ml-1">Gambar Cover (Opsional)</label>
                                <div class="relative group">
                                    <input type="file" name="image" id="image" class="hidden" onchange="previewImage(event)">
                                    <label for="image" class="block w-full px-5 py-8 border-2 border-dashed border-slate-200 bg-slate-50/30 rounded-2xl text-center cursor-pointer hover:border-indigo-300 hover:bg-slate-50 transition-all">
                                        <div id="preview-placeholder" class="{{ $berita->image ? 'hidden' : '' }} space-y-2">
                                            ...
                                            <div class="text-sm font-semibold text-slate-600">Klik untuk ganti gambar</div>
                                        </div>
                                        <img id="image-preview" src="{{ $berita->image ? Storage::url($berita->image) : '' }}" class="{{ $berita->image ? '' : 'hidden' }} w-full h-48 object-cover rounded-xl shadow-sm" alt="Preview">
                                    </label>
                                </div>
                                @error('image')
                                    <p class="text-rose-500 text-xs mt-1 font-semibold ml-1">{{ $message }}</p>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <label class="cursor-pointer">
                                        <input type="radio" name="status" value="draft" class="hidden peer" {{ old('status', $berita->status) === 'draft' ? 'checked' : '' }}>
                                    ...
                                    <label class="cursor-pointer">
                                        <input type="radio" name="status" value="published" class="hidden peer" {{ old('status', $berita->status) === 'published' ? 'checked' : '' }}>
                                        <div class="p-4 text-center border-2 border-slate-100 rounded-2xl bg-slate-50/30 text-slate-400 font-bold text-sm peer-checked:border-indigo-500 peer-checked:bg-indigo-50 peer-checked:text-indigo-600 transition-all">
                                            PUBLISHED
                                        </div>
                                    </label>
